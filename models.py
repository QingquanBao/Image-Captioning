...
class Scheduler:
    def __init__(self, method, decay_rate=None):
        self.method = method
        self.decay_rate = decay_rate
        self.steps = 0
        assert self.method in ['linear', 'exp', 'inv-sigmoid', 'none'],  method + " is NOT implemented now"

    def rate(self):
        if self.method == 'linear':
            rate_ = 1 - self.steps * self.decay_rate
        elif self.method == 'exp':
            rate_ = self.decay_rate ** self.steps
        elif self.method == 'inv-sigmoid':
            rate_ = self.decay_rate / (self.decay_rate + torch.exp(torch.tensor(self.steps / self.decay_rate)) )
            # When K=10, steps=45, rate = 0.1
            # When K=8,  steps=45, rate = 0.02
            # When K=5,  steps=45, rate = 0.001
    ...
    def step(self):
        self.steps += 1
        print('Scheduler Step, now rate is {}'.format(self.rate()))
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths, schedule_rate=1.):
        """
        Forward propagation.
        ...
        # Schedule Sampling Mask for embedding
        is_replace = (torch.rand((batch_size, max_len)) > schedule_rate).to(encoder_out.device)
...
class Captioner(nn.Module):
    def __init__(self, encoded_image_size, attention_dim, embed_dim,
        ...
        self.scheduler = Scheduler(kwargs['schedule_sampling'], kwargs['schedule_k'])
    ...
    def forward(self, images, encoded_captions, caption_lengths):
        ...
        decoder_out = self.decoder(encoder_out, encoded_captions,
                                   ...
                                   schedule_rate=self.scheduler.rate())
        return decoder_out
